...
def calc_entropy(router_probs, eps=1e-5):
    probs_neg_log = -torch.log(router_probs + eps)  # eps for 'p=0, -plogp=0'
    
    return (router_probs * probs_neg_log).sum(dim=-1)
...
class SwitchTransformersSparseMLP(nn.Module):
    ...
    def __init__(self, config: SwitchTransformersConfig, expert_class: nn.Module = SwitchTransformersDenseActDense):
        ...
        self.max_num_slots = config.max_num_slots
        self.gw_entropy_threshhold = config.gw_entropy_threshhold
        self.train_use_gw = config.train_use_gw
        self.eval_use_gw = config.eval_use_gw
        
    def forward(self, hidden_states):
        return self._forward(hidden_states, self.train_use_gw if self.training else self.eval_use_gw)
        
    def _forward(self, hidden_states, use_gw):
        router_mask, router_probs, router_logits = self.router(hidden_states)
        expert_index = torch.argmax(router_mask, dim=-1)

        if use_gw:
            router_entropy = calc_entropy(router_probs)  # (b_s, seq_len)
            gw_index = (router_entropy >= self.gw_entropy_threshhold).to(torch.int64)  # (b_s, seq_len)
            gw_index_mask = (torch.cumsum(gw_index, dim=-1) <= self.max_num_slots).to(torch.int64)
            gw_index = (gw_index * gw_index_mask).bool()
            gw_states = hidden_states[gw_index]
            
        next_states = hidden_states.clone()
        gw_next_states = torch.zeros_like(hidden_states)
        for idx, expert in enumerate(self.experts.values()):
            token_indices = router_mask[:, :, idx].bool()
            next_states[token_indices] = expert(hidden_states[token_indices]).to(next_states.dtype)
            
            if use_gw:
                router_probs_i = router_probs[gw_index][:, idx].unsqueeze(-1)  # (??, 1)
                gw_next_states[gw_index] = (router_probs_i * gw_states).to(gw_next_states.dtype)

        router_probs = torch.max(router_probs, dim=-1).values.unsqueeze(-1)
        if use_gw:
            next_states[gw_index] = 0.0
            hidden_states = router_probs * next_states + gw_next_states
        else:
...
class SwitchTransformersForConditionalGeneration(SwitchTransformersPreTrainedModel):
    ...
    def __init__(self, config: SwitchTransformersConfig):
        ...
        encoder_config.use_cache = False
        encoder_config.is_encoder_decoder = False
        encoder_config.train_use_gw = config.encoder_train_use_gw
        encoder_config.eval_use_gw = config.encoder_eval_use_gw
        encoder_config.max_num_slots = config.encoder_max_num_slots
        self.encoder = SwitchTransformersStack(encoder_config, self.shared)
        ...
        decoder_config.train_use_gw = config.decoder_train_use_gw
        decoder_config.eval_use_gw = config.decoder_eval_use_gw
        decoder_config.max_num_slots = config.decoder_max_num_slots
        self.decoder = SwitchTransformersStack(decoder_config, self.shared)
    ...
    def forward(
        ...
        encoder_entropy_avg = None
        encoder_gw_ratio = None
        decoder_z_loss = None
        decoder_aux_loss = None
        decoder_entropy_avg = None
        decoder_gw_ratio = None
        ...
        if output_router_logits:
            ...
            if self.encoder.config.encoder_sparse_step > 1:
                ...
                encoder_router_probs = torch.softmax(encoder_router_logits, dim=-1)
                encoder_entropy = calc_entropy(encoder_router_probs)
                encoder_entropy_avg = encoder_entropy.mean()
                encoder_gw_ratio = (encoder_entropy >= self.gw_entropy_threshhold).sum() / encoder_entropy.numel()
            else:
                encoder_z_loss = 0
                encoder_aux_loss = 0
                encoder_entropy_avg = 0
                encoder_gw_ratio = 0
            ...
            if self.decoder.config.decoder_sparse_step > 1:
                ...
                decoder_router_probs = torch.softmax(decoder_router_logits, dim=-1)
                decoder_entropy = calc_entropy(decoder_router_probs)
                decoder_entropy_avg = decoder_entropy.mean()
                decoder_gw_ratio = (decoder_entropy >= self.gw_entropy_threshhold).sum() / decoder_entropy.numel()
            else:
                decoder_z_loss = 0
                decoder_aux_loss = 0
                decoder_entropy_avg = 0
                decoder_gw_ratio = 0
        ...
        return Seq2SeqMoEOutput(
            ...
            encoder_entropy=encoder_entropy_avg,
            encoder_gw_ratio=encoder_gw_ratio,
            decoder_z_loss=decoder_z_loss,
            decoder_aux_loss=decoder_aux_loss,
            decoder_entropy=decoder_entropy_avg,
            decoder_gw_ratio=decoder_gw_ratio,
            past_key_values=decoder_outputs.past_key_values,
            decoder_hidden_states=decoder_outputs.hidden_states,
            decoder_attentions=decoder_outputs.attentions,
            cross_attentions=decoder_outputs.cross_attentions,
